        <!-- Sidebar Start -->
        <div class="sidebar pe-4 pb-3">
            <nav class="navbar bg-light navbar-light">
                <a href="index.html" class="navbar-brand mx-4 mb-3">
                    <h3 class="text-primary"><i class="fa fa-hashtag me-2"></i>Won Exam</h3>
                </a>
                <div class="d-flex align-items-center ms-4 mb-4">
                    <div class="position-relative">
                        <img class="rounded-circle" src="img/user.jpg" alt="" style="width: 40px; height: 40px;">
                        <div class="bg-success rounded-circle border border-2 border-white position-absolute end-0 bottom-0 p-1"></div>
                    </div>
                    <div class="ms-3">
                    @if(auth()->user())
                        <h6 class="mb-0">{{auth()->user()->name}}</h6>
                        <span>Admin</span>
                    @endif
                    </div>
                </div>
                <div class="navbar-nav w-100">
                    <a href="/dashboard" class="nav-item nav-link"><i class="fa fa-tachometer-alt me-2"></i>Dashboard</a>
                    <div class="nav-item dropdown">
                        <a href="#" class="nav-link dropdown-toggle" data-bs-toggle="dropdown"><i class="fa fa-laptop me-2"></i>Exams</a>
                        <div class="dropdown-menu bg-transparent border-0">
                            <a href="{{url('exam')}}" class="dropdown-item"> Exam Room</a>
                            <a href="{{url('exam')}}" class="dropdown-item">All Exam</a>
                            <a href="{{url('exam')}}" class="dropdown-item">Other Exam</a>
                        </div>
                    </div>
                    <a href="{{url('result')}}" class="nav-item nav-link"><i class="fa fa-th me-2"></i>Results</a>
                    <a href="{{url('resources')}}" class="nav-item nav-link"><i class="fa fa-keyboard me-2"></i>Resources</a>    
                    <a href="{{url('analytics')}}" class="nav-item nav-link"><i class="fa fa-chart-bar me-2"></i>Analytics</a>
                    <a href="{{url('help-center')}}" class="nav-item nav-link"><i class="fa fa-question-circle me-2"></i>Help Center</a>
                   
                </div>
            </nav>
        </div>